...
//var adv
String INPT, FLOW, FUPP, MODE, READ, CONF, SOUT;

char cINPT[7]; char cFLOW[4]; char cFUPP[4]; char cMODE[7]; char cREAD[4]; char cCONF[2]; char cSOUT[2];

//var info
String SERL, SREV, MODC, FIRC, FMAC;

char cSERL[13]; char cSREV[11]; char cMODC[10]; char cFIRC[10]; char cFMAC[5];
...
void setupWiFiAndServer() {
  WiFi.mode(WIFI_AP);
  //WiFi.begin("HugenPLUS-RADIO", NULL, 7, 0, 10);
  //Serial.println(WiFi.softAPmacAddress());
  String macAddress = String(ESP.getEfuseMac());
  //Serial.println(macAddress);
  //macAddress.replace(":", "");
  FMAC = macAddress.substring(macAddress.length() - 4);
  String ssid = "HugenPLUS-RADIO_" + FMAC;
  saveConfigFile(BTX_data);
  //Serial.println(FMAC);
  //WiFi.disconnect();
  ...
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    server.on("/adv", HTTP_POST, [](AsyncWebServerRequest *request){
      if (request->hasParam("modo",true)){
        ...
        AsyncWebParameter* fe = request->getParam("saidaonoff", true);
        MODE = fa->value();
        FLOW = fb->value();
        FUPP = fc->value();
        READ = fd->value();
...
void cliente(){
  ...
  attachInterrupt(digitalPinToInterrupt(BT1), handleInterrupt, CHANGE);
  WiFi.mode(WIFI_STA);
  WiFi.begin(WIFI.c_str(), WPAS.c_str());
  int i = 0;
  ...
  //switch(ntrip_c.reqSrcTbl(cHOST,iPORT,FMAC)){
  switch(ntrip_c.reqRaw(cHOST,iPORT,"",cUSER,cUPAS,"","",FMAC)){
  ...
  if(ChLoc=="S"){
    if(Latitude!=""&&Longitude!=""&&Altitude!=""&&Tempoutc!=""){
      ...
      switch(ntrip_c.reqRaw(cHOST,iPORT,cMNTP,cUSER,cUPAS,nmeaGGA,ChLoc,FMAC)){
        case 1: Serial.print("Não foi possivel conectar ao ");Serial.print(cHOST);Serial.print(":");Serial.println(iPORT);
                erroLedNtrip(1);
                clientmodo = 0;
                ERRC = "1";
    ...
    switch(ntrip_c.reqRaw(cHOST,iPORT,cMNTP,cUSER,cUPAS,"","",FMAC)){
      case 1: Serial.print("Não foi possivel conectar ao ");Serial.print(cHOST);Serial.print(":");Serial.println(iPORT);
              erroLedNtrip(1);
              clientmodo = 0;
              ERRC = "1";
  ...
  while(true){
    ...
    atualMillis = millis();
    if(atualMillis - milli_inicio > 15000){  
      ntrip_c.sendgga(enviaGGA);
      milli_inicio = millis();
    }
    
    if(atualMillis - currentMillis > 20000){
      for(int n=0; n<2; n++){
        delay(1);
        leds.setLedColorData(0, 0, 0, 0);
        leds.show();
...
void mserial(){
  ...
  delay(1);
  input = 0;
  Serial1.begin(SBUD.toInt(), SERIAL_8N1, RXDT, TXDT);
  Serial.print("SBUD: ");
  Serial.println(SBUD);
  //pinMode(TXDT,INPUT);
  //pinMode(RXDT,INPUT);
  digitalWrite(CONFP,HIGH);
  ext_com(HIGH);
  if(FlagOnly5V==false && SOUT== "S"){
    digitalWrite(POUTEN,HIGH);
  } else if (SOUT == "N") {
    digitalWrite(POUTEN,LOW);
  }
  while(true){
    while(Serial1.available()){
      delay(1);
...
void local(){
  ...
  Serial.println("modo local");
   WiFi.mode(WIFI_AP); 
   WiFi.softAP(WIFIL.c_str(),WPASL.c_str(),7,0,10);   //launch the access point
   Serial.println("Wait 100 ms for AP_START...");
   delay(100);
